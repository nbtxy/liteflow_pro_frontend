...
export function ConversationList() {
  ...
  const featureEntries = [
    ...
    },
    {
      ...
      titleKey: 'navConnectors' as const,
      icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1',
    },
    {
      path: '/chat/skills',
      titleKey: 'navSkills' as const,
      icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z',
    },
    {
      path: '/chat/schedules',
      titleKey: 'navSchedules' as const,
      icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
    },
  ];

  const renderFeatureEntry = (item: (typeof featureEntries)[number], compact: boolean) => (
    <button
      key={item.path}
      ...
      title={t.chat[item.titleKey]}
    >
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={item.icon} />
      </svg>
      {!compact && <span className="text-[10px] leading-tight">{t.chat[item.titleKey]}</span>}
    </button>
  );
    ...
    <div className="flex flex-col h-full">
      ...
      {desktopSidebarOpen ? (
        <div className="px-3 pb-3 grid grid-cols-4 gap-1.5">
          {featureEntries.map((item) => renderFeatureEntry(item, false))}
        </div>
      ) : (
        <div className="px-1.5 pb-2 space-y-1">
          {featureEntries.map((item) => renderFeatureEntry(item, true))}
        </div>
      )}
